...
if __name__ == "__main__":
    ...
    token_list_path = os.path.join(exp_dir, 'data', unit,
                                   'token_list')
    # 学習結果を出力するディレクトリ
    output_dir = os.path.join(exp_dir, unit+'_model_conv_non_ar_001')
    ...
    for epoch in range(max_num_epoch):
        ...
        for phase in ['train', 'validation']:
            ...
            for (features, labels, feat_lens,
                ...
                dec_input = labels[:,:].to(device)
                dec_target = labels[:,:].to(device)
                dec_input_lens = label_lens
                dec_input_lens = dec_input_lens.to(device)
                #dec_target = labels[:,:]
        ...
        for (features2, labels2, feat_lens2,
                   label_lens2, utt_ids2) in train_loader:
            if n_train >= 1:
                break
            n_train += 1
            ...
            #dec_input2 = labels2[:,:]
            #dec_input_lens2 = label_lens2 

            # モデルの出力を計算(フォワード処理)
            #outputs2, _ = model(features2.to(device), feat_lens2.to(device), dec_input2.to(device), dec_input_lens2.to(device) )
            #outputs3 = model.inference(features2.to(device), feat_lens2.to(device), labels2.to(device) )
            ...
            for n in range(outputs3.size(0)):
                ...
                # 各ステップのデコーダ出力を得る
                #_, hyp_per_step2 = torch.max(outputs2[idx2], 1)
                _, hyp_per_frame3 = torch.max(outputs3[idx3], 1)
                #hyp_per_frame3 = outputs3[idx3]
                #print( "size of hyp_per_frame3:{}".format( hyp_per_frame3.size() ))
                # numpy.array型に変換
                #hyp_per_step2 = hyp_per_step2.cpu().numpy()
                # numpy.array型に変換
                #hyp_per_frame3 = hyp_per.frame3.cpu().numpy()
                #print( "hyp_per_step2:{}".format( hyp_per_step2 ))
                # 認識結果の文字列を取得
                #hypothesis2 = []
                #for m in hyp_per_step2[:label_lens2[idx2]]:
                #    if m == eos_id:
                #        break
                #    hypothesis2.append(token_list[m])
        ...
        n_dev = 0
        for (features2, labels2, feat_lens2,
                   label_lens2, utt_ids2) in dev_loader:
            if n_dev >= 1:
                break
            n_dev += 1
            ...
            for n in range(outputs3.size(0)):
                ...
                print( "dev      reference:   ", *reference2, sep="" )                
                ...
                #hyp_per_step2 = hyp_per_step2.cpu().numpy()
                # numpy.array型に変換
                #hyp_per_step3 = hyp_per_step3.cpu().numpy()
                # 認識結果の文字列を取得
                #hypothesis2 = []
                #out_len2 = out_lens2[idx2].long()
                #for m in hyp_per_step2[:label_lens2[idx2]]:
                #    if m == eos_id:
                #        break
                #    hypothesis2.append(token_list[m])
                ...
                # numpy.array型に変換
                hyp_per_frame3 = hyp_per_frame3.cpu().detach().numpy()
                # 認識結果の文字列を取得
                hypothesis = \
                    ctc_simple_decode(hyp_per_frame3,
                                      token_list)
                                                                      
                #print( "dev      hypothesis:  ", *hypothesis2, sep="" )
                print( "dev      hypothesis:  ", *hypothesis, sep="" )
